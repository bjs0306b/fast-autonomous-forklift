...
| `cargo_id`, `measurement_id` | 운반 대상과 배치 판단 근거 |
| `vehicle_id` | 배정 차량, 배정 전 null |
| `destination_slot_code` | 선택한 적재 위치 |
| `destination_x`, `destination_y`, `destination_heading`, `fork_height` | 작업 생성 시점의 목적지 스냅샷 |
| `status` | 작업 진행 상태 |
| `assigned_at` ~ `failed_at`, `created_at` | 상태별 이력 시각 |

목적지 스냅샷은 슬롯 설정이 나중에 바뀌어도 이미 생성된 작업의 실행 목표를 보존한다.
