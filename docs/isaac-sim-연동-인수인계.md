...
세 항목 모두 정리됐다. 아래는 결론과 근거이고, 값 자체는 각 설정 파일이
소유한다.

### 2.1 MQTT 토픽 — 충돌이 아니라 **채널이 둘**이다

처음에 "두 계열 중 하나로 통일해야 한다" 고 적었는데, 그건 상황을 잘못 읽은
것이었다. 실차는 **두 계열을 동시에 발행하고 있고 용도가 다르다**:

| 계열 | 발행 주체 | 쓰는 쪽 |
|---|---|---|
| `forklift/{id}/status` · `location` · `path` · `command-result` | `bridge_core.py` | 백엔드 (관제 화면·DB·WebSocket) |
| `fast/v1/vehicle/{id}/telemetry` | `orin_telemetry.py` | 트윈 / 아이작 심 |

**아이작 심 쪽은 `fast/v1/...` 을 쓰면 된다.** 스펙은
`isaac_sim/docs/interface-spec.md` 가 소유한다. 백엔드 계열을 건드릴 필요가
없다 -- 그쪽은 이미 코드 200곳 이상이 붙어 있다.

### 2.2 각도 단위 — 아이작 심 채널은 **라디안**

이것도 2.1 과 같다. 채널마다 단위가 다르고, **변환은 이미 되어 있다**:

| 채널 | 필드 | 단위 | 변환 지점 |
|---|---|---|---|
| `fast/v1/...` (트윈·심) | `pose.yaw` | **라디안** −π ~ +π | `orin_telemetry.py` `normalize_angle` |
| `forklift/...` (백엔드) | `heading` | 도 [0, 360) | `dto.py` `quaternion_to_heading` |

`quaternion_to_heading` 이 `math.degrees()` 후 `normalize_heading()` 을 태운다.
처음에 "한쪽이 틀렸다" 고 적었지만 실제로는 각 채널이 자기 단위로 나가고 있다.

**아이작 심 쪽은 라디안을 그대로 쓰면 된다.** 백엔드의 도(degree) 표기는 이미
Java 코드 200곳 이상이 붙어 있어 건드릴 이유가 없다.

⚠️ 필드명도 다르다(`heading` vs `pose.yaw`). 같은 값이 아니라 **다른 채널의 다른
필드**라고 읽어야 한다 -- 한쪽 이름으로 다른 쪽을 찾으면 없다.

⚠️ 축척은 각도에 적용하지 않는다. 위치만 ×10 이고 yaw 는 그대로다.

### 2.3 `base_link` 기준점 — **전륜 구동축** 중심

| 출처 | 정의 | |
|---|---|---|
| `ros2_ws/README.md:76` | 전륜 구동축 중심 | 맞음 |
| `docs/센서-측정-칼리브레이션.md:39` | 전륜 구동축 중심 (REP-103) | 맞음 |
| `ros2_ws/orin-pose-spec.md:80` | 뒷바퀴 축 중심 | **오기 — 수정 필요** |

실물 코드와 모든 static TF 가 전륜 구동축 기준이고, 2026-08-07 에 실차에서
재확인했다. 후륜(조향)축은 `x = -0.144`(축거), footprint 뒤끝은 `-0.245` 이므로
조향축 뒤로 0.101 m 가 더 나온다.

이 기준이 어긋나면 **축거 0.144 m 만큼 밀리고, 회전할 때마다 어긋남의 방향이
바뀌어** "가끔 밀린다" 로 보인다. 트윈을 만들 때 가장 먼저 확인할 것.

⚠️ 이 기준점은 회전 기하 전체의 뿌리다. `tools/geometry_limits.py` 가 여기서
꼬리 휨(0.182 m)과 최소 회전반경(0.198 m)을 유도한다.
...
시뮬 차량 모델에 넣을 값 일체는 **현재 설정에서 뽑아 쓴다**:

```bash
cd ros2_ws
python3 tools/geometry_limits.py --handover
```

좌표 규약, 치수, 조향 기하(최소 회전반경·꼬리 휨), 속도 범위, 안전거리,
축척, 그리고 각 값이 사는 파일까지 한 번에 나온다.

⚠️ **출력을 문서에 붙여 넣고 끝내지 말 것.** 설정이 바뀌면 다시 돌려서
갈아끼운다. 옮겨 적은 숫자가 낡아서 생긴 사고가 이미 있었다 --
`max_linear_mps` 가 바뀌자 포크 진입이 **조용히** 파렛트를 과하게 밀기
시작했고, 로그에는 정상으로 찍혔다(§5 참고).
