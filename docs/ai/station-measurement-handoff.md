...
`serve.py`는 이때 종료 코드 **2**로 끝나 잠김을 알린다. 코드 체계는 이렇다:

| 코드 | 뜻 | 볼 곳 |
|---|---|---|
| `0` | 성공 | — |
| `1` | **재려고 했는데 실패** (검출 없음·전송 실패) | 화물 배치·조명·검출 |
| `2` | **내가 연 세션이 안 풀림** | `--release-session [--abandon]` |
| `3` | **세션을 못 열어 시작조차 못 함** (409 점유·백엔드 불통) | 설비 점유 여부·백엔드 연결 |

⚠️ `1`과 `3`은 **운영자가 할 일이 다르다.** 종전에는 둘 다 `1`이라 로그 끝만
보고는 "재봤는데 안 나온 건지, 아예 못 재본 건지"를 가릴 수 없었다
(2026-08-03 분리).
