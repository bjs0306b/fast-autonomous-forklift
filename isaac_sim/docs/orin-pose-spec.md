...
관제가 `{"x": 16.5, "y": 5.0}` 을 주면 → 실제로는 (1.65, 0.50) m 로 간다.
...
| `forklift/fk01/arrived` | **입고 바이에 도착해 멈췄을 때** | `{"ts":..., "vehicleId":"fk01", "position":{"x":16.5,"y":5.0}}` |
...
{ "taskId": "T-0042", "ts": 1785000000000, "action": "GOTO",
  "pickup": { "x": 16.5, "y": 5.0, "yaw": 0.0 } }
```

받으면 → `(1.65, 0.50) m` 로 환산해 자기 Nav2 에 `navigate_to_pose` 를 넣는다.
