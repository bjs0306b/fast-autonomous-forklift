...
| 순서 | 모서리 | 좌표 | 실물 m | **떠날 때 yaw** | 다음 구간 |
|---|---|---|---|---|---|
| 1 | **C0** | 15.5, 4.0 | 1.550, 0.400 | **+1.5708** | 오른쪽 통로를 북으로 |
| 2 | **C1** | 15.5, 27.0 | 1.550, 2.700 | **3.1416** | 위 통로를 서로 |
| 3 | **C2** | 5.0, 27.0 | 0.500, 2.700 | **−1.5708** | 왼쪽 통로를 남으로 |
| 4 | **C3** | 5.0, 4.0 | 0.500, 0.400 | **0** | 아래 통로를 동으로 |

각 변 길이는 23.0 / 10.5 / 23.0 / 10.5, 둘레 **67.0**.

`task` 로 보낼 때 `yaw` 는 **그 모서리에 도착한 뒤 나아갈 방향**입니다.
예를 들어 C1 로 보낼 때는 오른쪽 통로를 북으로 가는 중이므로 `1.5708`.

```json
{ "taskId": "T-1", "x": 15.5, "y": 27.0, "yaw": 1.5708 }
```

```java
// 반시계 순서. Track 생성자에 그대로 넣습니다.
static final double[][] CORNERS = {
    {15.5,  4.0},   // C0
    {15.5, 27.0},   // C1
    { 5.0, 27.0},   // C2
    { 5.0,  4.0},   // C3
};
static final double[] CORNER_YAW = {
     Math.PI / 2,   // C0 -> 북
     Math.PI,       // C1 -> 서
    -Math.PI / 2,   // C2 -> 남
     0.0,           // C3 -> 동
};
static final double LOOP_LENGTH = 67.0;
```
...
| 구간 | 위치 | 용도 |
|---|---|---|
| C0→C1 | x = 15.5 (오른쪽 통로) | **B랙 접근** (x=14.5 에서 빠짐) |
| C1→C2 | y = 27.0 (위 통로) | 이동만 |
| C2→C3 | x = 5.0 (왼쪽 통로) | **A랙 접근** (x=5.0 이 곧 접근점) |
| C3→C0 | y = 4.0 (아래 통로) | **바이 진입** (16.5, 5.0) |
...
모서리를 한 번에 다 주지 않고, **도착하면 다음 하나**를 줍니다.

```
관제 ──task──▶ 차량 ──navigate_to_pose──▶ Nav2 ──▶ 주행
     모서리 하나            차량이 경로계획·장애물회피
```

미리 다 주면 중간에 개입할 수 없기 때문입니다. 관제는 매 tick 마다
다시 판단합니다 — 앞차가 가까우면 목표 대신 `HOLD`, 바이가 점유됐으면
모서리를 하나 더 줘서 한 바퀴 더 돌립니다.

`navigate_through_poses` 로 경로 전체를 넘기는 방법도 있지만, 중간에
멈추거나 바꾸기가 어려워 쓰지 않습니다.

도착 판정은 **telemetry 위치**로 합니다.

```java
boolean arrived(Vehicle v, double[] target, double tol) {
    return Math.hypot(v.x - target[0], v.y - target[1]) < tol;
}
```
...
|---|---|
| 모서리 | `CORNER_TOL` 2.0 |
| 스테이션(바이·랙 접근점) | `ARRIVE_TOL` 1.5 |

`event: TASK_COMPLETED` 도 오지만 **그것에 의존하지 마세요.** 이벤트가
유실되면 그 차량은 영영 다음 목표를 받지 못합니다. 이벤트는 더 빠른
보조 신호로만 쓰고, 판정은 위치로 합니다.
...
| `ALIGN_BAY` | `cargo` ← `{"action":"align_bay"}` | **7초 경과 `AND` `busy == false`** |
...
### ALIGN_BAY 를 넘기는 조건 — 주의

**`state != LOADING` 으로 판정하면 안 됩니다.** 지시를 보낸 직후에는 아직
차량이 반영하기 전이라 `IDLE` 이고, 그러면 정렬을 시작하지도 않았는데
"끝났다" 로 오판합니다.

지시부터 `state` 가 `LOADING` 으로 바뀌기까지 **약 200~400 ms** 걸립니다.

```
백엔드 publish  →  브로커  →  브릿지  →  ROS  →  Isaac 다음 프레임
                →  /state 발행(10 Hz)  →  브릿지 telemetry(10 Hz)  →  백엔드
```

올바른 조건은 **둘 다** 만족입니다.

```java
boolean alignDone(Vehicle v) {
    if (now() - v.phaseStartedAt < ALIGN_MIN_MS) return false;   // 7000
    return !v.busy;              // telemetry 의 busy 필드
}
```

| 상수 | 값 | 뜻 |
|---|---|---|
| `ALIGN_MIN_MS` | 7000 | 최소 대기. 정렬 자체가 최대 8초 걸린다 |
| `ALIGN_MAX_MS` | 12000 | 이 시간이 지나면 `busy` 와 무관하게 진행 |

`busy` 는 **차량이 절차(정렬·도킹·적재)를 수행 중인가**를 나타냅니다.
telemetry 에 실려 옵니다.

```json
{ "state": "LOADING", "busy": true, "step": "align", ... }
```

| 필드 | 정렬 중 | 정렬 끝 |
|---|---|---|
| `state` | `LOADING` | `LOADING` (화물을 받을 때까지 유지) |
| **`busy`** | **true** | **false** ← 이걸로 판정 |
| `step` | `"align"` | `""` |

`state` 는 정렬이 끝나도 `LOADING` 을 유지합니다 (적재 절차 전체를 덮으므로).
그래서 **단계 전환은 `busy` 로** 판정해야 합니다.

> `busy` / `step` 은 시뮬(sim02·sim03)만 보냅니다. 실물(fk01)은 C팀이
> 구현하기 전까지 이 필드가 없으므로, **없으면 최소 대기 시간만으로**
> 판정하세요.

```java
boolean alignDone(Vehicle v) {
    long el = now() - v.phaseStartedAt;
    if (el < ALIGN_MIN_MS) return false;
    if (el > ALIGN_MAX_MS) return true;        // 안전장치
    return !v.hasBusyField || !v.busy;         // 필드가 없으면 시간만으로
}
```
