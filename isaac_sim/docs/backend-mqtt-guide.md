...
> ## ⚠️ 2026-08-06 레포 대조 — 백엔드·ROS2 는 아직 이 규격이 아니다
>
> 이 문서는 **트윈 쪽에서 백엔드에 요청하는 규격**입니다. 실제 코드와 맞춰본 결과
> **세 가지가 지금 어긋나 있습니다.** 읽는 사람이 "이미 붙어 있다" 고 오해하지 않도록 적습니다.
>
> | | 이 문서 | 지금 코드 |
> |---|---|---|
> | 토픽 | `fast/v1/vehicle/{id}/telemetry` 등 | `forklift/{vehicleId}/status` · `/location` · `/command` · `/command-result` |
> | 차량 ID | `fk01` · `sim02` · `sim03` | `REAL-F01` · `SIM-F01` (**하이픈**, DB `vehicle.vehicle_id` 와 같아야 함) |
> | 좌표 | 시뮬 좌표 0~20 · 0~30 | 실물 m — `fast_mqtt_bridge` 에 ×10 코드가 없습니다 |
>
> `fast/v1/...` 을 쓰는 코드는 `isaac_sim/nav2/scripts/` 3개(트윈 내부)뿐입니다.
> 백엔드는 `forklift/+/...` 를 구독하므로 **지금 트윈이 이 규격으로 쏘면 백엔드에
> 아무것도 안 들어갑니다.** 에러도 안 납니다 — 구독이 없는 토픽은 그냥 버려집니다.
>
> ⚠️ 차량 ID 가 특히 조용합니다. 백엔드는 **미등록 vehicleId 를 경고 로그만 남기고
> 버립니다**(`VEHICLE_NOT_FOUND`). `fk01` 로 보내면 연결된 것처럼 보이지만 저장이 안 됩니다.
>
> **어느 쪽으로 통일할지는 협의 사항입니다**(이준서 ↔ 전지웅·김재원). 이 대조는
> "무엇이 다른가" 만 기록한 것이고 규격의 옳고 그름을 판단한 것이 아닙니다.
> 관련: S15P11A304-92 · 111 · 112 · 158.
...
```json
{
  ...
  "dropoff": { "x": 5.0,  "y": 9.3,  "yaw": 3.1416, "forkHeight": 13.25 }
}
```
...
선반 높이(포크 목표)는 `13.25`. 진입 방향 yaw 는 `3.1416`(서쪽).
