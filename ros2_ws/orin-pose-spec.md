...
| 기준점 | 차체의 **base_link** — **전륜 구동축** 중심 |
| forkHeight | 이것도 **×10** 해서 보낸다 (실물 0.02 m → `0.2`) |

`yaw` 는 각도라 축척과 무관하다. 그대로 보낸다.

> ⚠️ **2026-08-07 정정: 기준점은 전륜 구동축이다.** 이 문서는 "뒷바퀴 축" 이라고
> 적고 있었는데 오기였다. 실물 코드와 모든 static TF 가 전륜 구동축 기준이고,
> `ros2_ws/README.md` 와 `docs/센서-측정-칼리브레이션.md` §1 도 그렇게 적고 있다.
>
> 이 기준이 어긋나면 **축거 0.144 m 만큼 밀리고, 회전할 때마다 어긋남의 방향이
> 바뀌어** "가끔 밀린다" 로만 보인다 -- 증상으로 원인을 찾기 어렵다. 후륜(조향)축은
> `x = -0.144`, footprint 뒤끝은 `-0.245` 라 조향축 뒤로 0.101 m 가 더 나온다.
>
> 이 문서를 근거로 만든 것이 있으면 함께 확인할 것.
