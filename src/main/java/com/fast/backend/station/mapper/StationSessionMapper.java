...
/** {@code station_session}과 단일 {@code station_state} 행에 접근한다. */
@Mapper
public interface StationSessionMapper {
    ...
    /**
     * 설비를 점유한다. 유휴이거나 <b>TTL 이 만료된</b> 점유일 때만 성공한다. 만료 판정과 점유를
     * 조건부 UPDATE <b>한 문장</b>으로 처리하므로 두 요청이
    ...
    /**
     * 점유 상태를 <b>행 잠금과 함께</b> 읽는다. 측정 저장이 "확인 → INSERT" 사이에
