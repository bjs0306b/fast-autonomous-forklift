...
> 🔴 **단, 그건 펌웨어까지의 이야기입니다** (2026-08-06 추가). ROS2 쪽에서
> `obstacle_avoidance` guard 를 띄우면 **ToF 가 하나만 죽어도 차가 안 갑니다** —
> `require_both_front_tof: true` 라 0.5초간 데이터가 없으면 `SENSOR_TIMEOUT` 으로
> 0 Twist 를 냅니다. 둘 다 의도된 동작이지만 방침이 반대라, 이 문장만 읽고
> "ToF 없어도 굴러간다" 고 판단하면 원인을 모터에서 찾게 됩니다.
>
> 증상이 "명령은 나가는데 안 움직임" 일 때 먼저 볼 것:
> `ros2 topic echo /obstacle_avoidance/status` — `reason` 에 어느 센서인지 나옵니다.
