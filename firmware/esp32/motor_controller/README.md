...
| ESP32 → Jetson | **네이티브 USB CDC** (`/dev/ttyACM*`) | `@IMU` 100Hz, `@IMS` 1Hz, `@TOF` 센서당 15Hz |
...
두 센서 모두 기본 주소 0x29라 순차 초기화가 필요합니다.

1. PWREN을 토글해 **하드웨어 리셋** — ESP32만 재부팅되면 센서는 이전 실행의 주소를
   그대로 갖고 있어, 이 단계 없이는 좌측 센서를 못 찾습니다
2. LPn으로 우측을 침묵시키고 좌측만 깨움
3. 좌측을 0x2A로 이설
4. 우측을 0x29로 합류

**주의**: `vl53l8cx_set_i2c_address()`는 `platform.address`만 바꾸고 I2C 디바이스
핸들은 그대로 둡니다. 전송은 핸들로 나가므로 **호출 뒤 핸들을 재등록**하지 않으면
통신이 끊깁니다 (`vl53l8cx_pair.c`의 `tof_bind_handle`).
