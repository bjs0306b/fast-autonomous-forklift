...
│  ├─ geometry.py          bbox(px) + 거리 → 실치수 (핀홀 모델)
│  └─ size_grade.py        크기 등급 판정·미니어처 매핑
├─ tests/                  테스트
...
검출된 박스를 **크기 등급**으로 나누고, 대응하는 미니어처 화물로 매핑한다
(명세 2.1: 실제 화물 3개 ↔ 미니어처 화물 3개, 디지털 트윈 적재).

```bash
cd ai
python -m perception.size_grade --config configs/cargo.yaml   # 설정 검증
```

### 동작

```
bbox(px) + ToF 거리  →  정면 실치수(cm)  →  등급  →  미니어처
        geometry.py            size_grade.py
```

핀홀 모델로 `실제크기 = 픽셀크기 × 거리 / 초점거리(px)`를 계산한다. 거리는
VL53L8CX ToF(FR-103)에서 받으므로 **ToF 없이는 치수를 낼 수 없다.**
...
**2. 어느 면이 보일지 모른다.** 27×18×15 박스는 27×18로도, 27×15로도, 18×15로도
보인다. 그래서 등급마다 세 면을 모두 후보로 두고 관측값과 대조한다.

### ⚠️ 화물 3종 선정 제약

**크기만 보고 고르면 안 된다.** 어느 면이 보일지 모르므로, 서로 다른 등급의 면
치수가 겹치면 구분이 불가능해진다.

우체국 택배 표준 규격 20개 조합을 전수 비교한 결과:
...
| 1호 / 2호 / 5호 | 18.5% | 20% 미달 |
| 1호 / 2호 / 3호 | 12.0% | 위험 |
| 2-1호 / 3호 / 4호 | 2.9% | 사실상 구분 불가 |

'구분 여유'는 **측정 오차가 이 값을 넘으면 오판이 시작되는 지점**이다. 1·2·3호처럼
인접한 호수를 고르면 12%밖에 안 되는데, ToF 거리 오차와 bbox 경계 오차가 함께
실리면 쉽게 넘는다.

현재 채택안은 2-1호(35×25×10)가 납작해 나머지 둘과 확실히 갈린다.

> **치수는 임시값이다.** 명세는 "실제 화물 3개를 준비한다"고만 하고 규격을 정하지
> 않았다. 팀이 실물을 확정하면 `configs/cargo.yaml`의 `dimensions_cm`을 실측값으로
> 교체하고 **반드시 검증 명령을 돌린다.** 코드 수정은 필요 없다.

판정이 애매하면(허용 오차 초과) 억지로 등급을 붙이지 않고 **미지로 둔다.**
잘못된 미니어처를 적재하면 디지털 트윈이 실물과 어긋나기 때문이다.
...
- **화물 3종 실물 확정**: `configs/cargo.yaml`의 치수는 임시값이다. 실물을 정하면
  실측값으로 교체하고 구분 여유를 재검증한다.
- **카메라 캘리브레이션**: 현재 화각(70도) 기반 근사라 정확도가 낮다. Jetson 배포
  (FR-101-5) 때 체스보드 캘리브레이션 값으로 교체한다.
- **FR-103과의 경계**: FR-102·FR-103 모두 Task에 '미니어처 대응 매핑'이 적혀 있다.
  현재는 FR-102가 등급 정의와 매핑표를, FR-103이 높이·용적 측정을 맡는 것으로
  나눴다. 전지웅 님과 확인이 필요하다.
