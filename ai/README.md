...
센서다. 존들의 거리에 평면을 맞추면 면의 기울기까지 나오고, 그게 두 문제를
동시에 푼다.

**1. 기울기 왜곡.** 정면에서 θ만큼 틀어진 면은 `cos θ`만큼 작게 보인다. 30°면
13% 축소인데 등급 구분 여유가 26.5%라 그대로 두면 오판이 된다. 좌우 기울기는
가로를, 상하 기울기는 세로를 줄이므로 **둘을 따로 구해 각각 보정한다.**

**2. 엉뚱한 대상 측정.** 평면 잔차가 크면 평평한 박스 면이 아니라 여러 물체나
빈 공간을 보고 있다는 뜻이다. 이때는 판정을 포기한다. 기울기가 45°를 넘어도
보정 배율이 커져 신뢰할 수 없으므로 거부한다.

거리는 존 값의 중앙값이 아니라 **평면 맞춤에서 얻은 광축 방향 수직 거리**를
쓴다. ToF 존은 광선 방향 거리를 재므로 정면 평면이라도 가장자리 존이 더 멀게
나오고, 중앙값을 그대로 쓰면 5%쯤 과대추정된다.

> 평면 맞춤은 3×3 정규방정식을 크라메르 공식으로 직접 푼다. BLAS/LAPACK을 쓰지
> 않으므로 numpy 백엔드 상태나 Jetson의 BLAS 빌드에 영향받지 않는다. 점이 64개라
> 성능상으로도 이쪽이 낫다.
...
**3. 어느 거리가 어느 bbox의 것인지는 아직 미해결.** 카메라와 ToF는 위치가 다른
별개 센서다. 두 센서의 외부 파라미터를 맞추지 않으면 왼쪽 박스의 bbox에 오른쪽
박스까지의 거리를 적용할 수 있다. 데모 범위에서는 **픽업 접근 순간(대상이 정면
중앙에 오는 시점)에만 판정**하는 운용으로 회피한다. 밀집 장면에서 여러 박스를
동시에 판정하려면 캘리브레이션이 필요하다.
