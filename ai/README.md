...
├─ src/perception/         실시간 인지 로직
│  └─ geometry.py          치수 측정(핀홀) + 미니어처 환산
├─ tests/                  테스트
...
핀홀 모델로 `실제크기 = 픽셀크기 × 거리 / 초점거리(px)`를 계산한다. 거리 없이는
크기를 알 수 없다 — 같은 200px bbox라도 거리가 50cm면 11cm, 200cm면 44cm다.

### 화물 치수를 코드에 넣지 않는다

정해둔 값과 대조하는 건 인식이 아니라 조회다. `perception` 모듈에는 **어떤 화물
치수 상수도 없다.** 박스는 아무거나 쓰고, 시스템은 재서 알아낸다. 미니어처는 실물의
1/10로 만든다.

실측 정확도를 확인할 때만 사람이 자로 잰 값과 비교하며, 그 값은 검증 문서에만 둔다
(test 세트를 미리 떼어두는 것과 같은 논리 — 정답을 아는 건 평가할 때만).

> **제약**: 미니어처가 11×11cm 파렛트(T-11 표준 1,100×1,100mm의 1/10)에 올라가야
> 하므로 실물 박스 바닥 면적이 110cm를 넘으면 안 된다.

### 측정값에 불확실성을 함께 싣는다

`3.42cm`가 아니라 `3.42±0.17cm`를 돌려준다. 적재 배치가 안전 마진을 잡으려면
오차 크기를 알아야 하는데, 치수만 넘기면 배치 쪽에서 추측하게 된다.

오차는 두 갈래로 들어온다:

| 원인 | 성질 | 크기 |
|---|---|---|
| 거리계 오차 | 비율 그대로 크기 오차가 됨 | [TF-Nova](https://sensorlidar.com/products/benewake-tf-nova-lidar-compact-14m-precision) ±3cm → 50cm에서 6%, 100cm에서 3% |
| bbox 경계 흔들림 | 거리가 멀수록 커짐 | 검출기 성능에 따라 |

독립이라 보고 제곱합으로 합친다. 실제 예상 구간(60~100cm 촬영)에서 **3~5%**,
34cm 박스면 ±1~2cm, 미니어처로는 ±1~2mm다.

### 한계

**1. 깊이는 관측되지 않는다.** 단일 시점에서는 정면에 보이는 두 변만 잴 수 있다.

**2. 기울기를 보정할 수 없다.** 비스듬히 본 면은 `cos θ`만큼 작게 보이는데, TF-Nova는
단일 점 거리계라 기울기를 알 방법이 없다. **박스를 카메라에 정면으로 두는 운용**으로
대응한다. 책상 고정 리그라면 어렵지 않다.

**3. 어느 거리가 어느 bbox의 것인지.** 화면에 박스가 여럿이면 거리계 값이 어느 것의
것인지 알 수 없다. FoV가 14°×1°로 좁아 사실상 정면 한 지점만 재므로, **한 번에 한
박스만** 다루는 운용이 전제다.

> 카메라가 고정 리그라면 **호모그래피** 방식이 더 정확할 수 있다. 책상면과 카메라
> 관계를 한 번 캘리브레이션해 두면 박스 바닥 접점 픽셀만으로 거리가 나와, 거리계의
> ±3cm를 아예 겪지 않는다. 검토 대상.
...
- **카메라 캘리브레이션**: `PinholeCamera.from_fov`는 화각 기반 근사라 정확도가
  낮다. 실제 운용 전 체스보드 캘리브레이션 값으로 교체한다.
- **호모그래피 방식 검토**: 고정 리그라면 거리계 없이 더 정확할 수 있다.
- **FR-103과의 경계**: FR-102·FR-103 모두 Task에 '미니어처 대응 매핑'이 적혀 있다.
  치수 측정은 FR-102, 높이·용적은 FR-103으로 나눴으나 전지웅 님과 확인이 필요하다.
- **측정 오차와 적재 밀도**: 측정 오차가 3~5%라 빡빡한 배치는 충돌한다. 배치 계획은
  불확실성만큼 안전 마진을 두고, 적재율을 100%까지 밀어붙이지 않는다.
