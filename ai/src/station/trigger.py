...
⚠️ **임계는 반드시 센서 노이즈 바닥보다 위여야 한다.** 처음에 2.0으로 잡았다가
**판정이 아예 동작하지 않았다** — BRIO 1920×1080의 노이즈 바닥이 2.62~3.44라 가만히
있어도 100% "안 멎었다"로 빠졌다(2026-07-31 실측). 짐작하지 말고 재야 한다.

실측 근거 (BRIO 1920×1080, 실내광):

    상태            움직임 값
    정지(노이즈)    2.62 ~ 3.44   (중앙값 2.77)
    1px 이동        3.09          ← 노이즈와 구분 불가
    2px 이동        5.00
    3px 이동        6.25
    5px 이동        7.99
    10px 이동      11.04

6.0은 노이즈 최대(3.44)의 **1.75배**라 여유가 있고, 3px 이상 움직임을 잡는다. 1~2px
흔들림은 놓치지만 1920px 폭에서 2px는 미니어처 환산 0.1mm 미만이라 KPI(4mm)에 무의미하다.
정작 막아야 할 것은 의자가 아직 구르거나 손이 프레임에 걸린 경우이고, 그건 값이 수십이다.

⚠️ **카메라·조명이 바뀌면 다시 재라.** 어두우면 센서 노이즈가 올라간다.
`serve.py --check`가 노이즈 바닥과 임계를 같이 찍어준다."""
...
class MeasureTrigger:
    ...
    def __enter__(self) -> "MeasureTrigger":
        # **멱등하다.** 호출자가 연결 실패를 먼저 확인하려고 `__enter__()`를 직접 부른 뒤
        # `with`로 감쌀 수 있어야 한다 — 두 번 연결하면 구독이 중복되고 요청이 두 번 온다.
