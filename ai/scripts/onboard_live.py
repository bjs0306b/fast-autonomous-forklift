...
⚠️ **젯슨에서는 1280x800이 안 나온다. 최대가 1280x720이다.** 그래서 젯슨에서 이
스크립트를 돌리면 "학습·평가와 다른 해상도다" 경고가 **항상** 뜬다 — 정상이다.
2026-07-31 라이브 검증에서 720으로 30프레임 전부 `pallet 0.93 + hole 0.83·0.89`,
15.5fps 로 정상 검출됐다. 노트북(1280x800 지원)에서는 경고가 안 뜬다.

경고가 늘 뜨는 상태를 그대로 두는 이유는, 이 경고가 **정말 이상한 해상도**(예:
640x480 으로 떨어진 경우)도 같이 잡아주기 때문이다. 실제 값을 눈으로 볼 것.
"""
