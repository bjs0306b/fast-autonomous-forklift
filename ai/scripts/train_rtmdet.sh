...
MODE=${1:-full}
GPU=${GPU:-1}
CONFIG=${CONFIG:-configs/rtmdet_s_forklift.py}
WORKDIR=${WORKDIR:-work_dirs/rtmdet_s_forklift}
mkdir -p "$WORKDIR"

# 실험별 override: CONFIG=configs/rtmdet_m_forklift.py WORKDIR=work_dirs/rtmdet_m_loco_cardboard LR=5e-5 bash scripts/train_rtmdet.sh
EXTRA_OPTS=()
if [ -n "${LR:-}" ]; then
  EXTRA_OPTS+=(optim_wrapper.optimizer.lr="$LR")
fi
...
if [ "$MODE" = "smoke" ]; then
  ...
  if [ ${#EXTRA_OPTS[@]} -gt 0 ]; then
    CUDA_VISIBLE_DEVICES=$GPU nohup mim train mmdet "$CONFIG" --gpus 1 \
      --work-dir "$WORKDIR" --cfg-options "${EXTRA_OPTS[@]}" > "$LOG" 2>&1 &
  else
    CUDA_VISIBLE_DEVICES=$GPU nohup mim train mmdet "$CONFIG" --gpus 1 \
